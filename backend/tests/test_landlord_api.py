...
These exercise the full request flow for real: FastAPI route -> service ->
repository -> SQL Server -> response, via FastAPI's TestClient (no mocking
of any layer). Like test_landlord_service.py, write tests clean up their
own throwaway rows so the seeded demo dataset stays exactly as seeded for
...
def test_list_landlords_returns_paginated_envelope() -> None:
    response = client.get("/api/landlords", params={"page": 1, "page_size": 2})
...
def test_list_landlords_search_filters_results() -> None:
    response = client.get("/api/landlords", params={"search": "Green Oak"})
...
def test_get_landlord_not_found_returns_standard_error_shape() -> None:
    response = client.get("/api/landlords/999999")
...
def test_full_landlord_lifecycle_via_api() -> None:
    ...
    try:
        # Create
        create_response = client.post("/api/landlords", json=_create_payload())
        ...
        dup_response = client.post("/api/landlords", json=_create_payload(CompanyName="Another Name Ltd"))
        assert dup_response.status_code == 409
        assert dup_response.json()["error"]["code"] == "DUPLICATE_EMAIL"

        # Get
        get_response = client.get(f"/api/landlords/{created_id}")
        ...
        status_response = client.patch(f"/api/landlords/{created_id}/status", json={"IsActive": False})
        assert status_response.status_code == 200
        assert status_response.json()["IsActive"] is False

        # Reactivate
        reactivate_response = client.patch(f"/api/landlords/{created_id}/status", json={"IsActive": True})
        assert reactivate_response.status_code == 200
        assert reactivate_response.json()["IsActive"] is True

        # Delete (soft - no active properties, so it succeeds)
        delete_response = client.delete(f"/api/landlords/{created_id}")
        assert delete_response.status_code == 204
        assert delete_response.content == b""

        # The record still exists, just inactive - this was a soft delete.
        final_get = client.get(f"/api/landlords/{created_id}")
