...
Last updated: 2026-08-06, after completing the Rent Payments frontend module (Prompt 22).
...
**Frontend: React foundation + reusable component library + all four repeatable-CRUD modules + Tenancies + Rent Payments done** (`frontend/`, plain React + JavaScript + CSS, Vite, react-router-dom, axios - per the scope doc's explicit "use plain React and readable CSS", no TypeScript/Tailwind/component library). Routing, API client with silent-refresh-on-401, auth context, protected routes, sidebar+header layout, all 15 of Prompt 19's reusable components, Prompt 20's four CRUD modules, Prompt 21 (Tenancies), and now Prompt 22 (Rent Payments - a second bespoke-lifecycle module, plus the app's first client-side CSV export). See "Frontend foundation", "Reusable component library", "Landlords frontend module (Prompt 20)", "Properties frontend module (Prompt 20)", "Tenants frontend module (Prompt 20)", "Employees frontend module (Prompt 20)", "Tenancy frontend module (Prompt 21)", and "Rent Payments frontend module (Prompt 22)" below.

**Not started yet:** the remaining frontend business module (Maintenance) and the real Dashboard page; deployment.

## Next steps, in order

Follow `documentation/project-scope.md`'s own sequence (section 57 / the numbered prompts):

1. **Prompt 23: Maintenance frontend** (see the doc for its specific requirements - filters by status/priority/category/property/employee, assign-employee, update-status - its own bespoke shape like Tenancies/Rent Payments, not the generic CRUD template).
2. **Then the Dashboard frontend** (KPI cards, charts, report filters, CSV export) — replaces the placeholder `HomePage.jsx`, and is the first real consumer of `KpiCard`. Can reuse `utilities/csvExport.js` (built for Rent Payments) rather than inventing a second CSV mechanism.
...
## Rent Payments frontend module (Prompt 22) - a second bespoke-lifecycle module, plus the app's first CSV export

Same "no generic status PATCH, dropdown filters instead of search" shape as Tenancy (`RentPaymentRepository.list` has no free-text search either - see `rentPaymentService.js`'s docstring). New files: `services/rentPaymentService.js`, `pages/rent-payments/{RentPaymentsList,RentPaymentDetail,RentPaymentForm,RentPaymentOverdue,RentPaymentDueThisMonth}Page.jsx`, `constants/rentPaymentOptions.js` (`PAYMENT_METHOD_OPTIONS` - a real form field on record-payment; `PAYMENT_STATUS_OPTIONS` - filter-only, same reasoning as `TENANCY_STATUS_OPTIONS`). `roles.js` gained `CAN_VIEW_RENT_PAYMENTS`/`CAN_MANAGE_RENT_PAYMENTS` (same Landlords-shaped Administrator+PropertyManager+ReadOnly / Administrator+PropertyManager split), `Sidebar.jsx`'s `Rent Payments` entry went live.

**`RentPaymentDetailPage` shows `AmountDue`/`AmountPaid`/`AmountOutstanding` as three separate, clearly-labeled `detail-grid` fields** (Prompt 22's explicit "clearly distinguish amount due, amount paid and amount outstanding" requirement) rather than folding them into one summary line. Its Edit link mirrors `update_payment`'s own two checks exactly - only shows while `AmountPaid` is still 0 AND the payment isn't Cancelled - since once any money is recorded, correcting the obligation itself is no longer a plain edit (cancel and recreate instead).

**The record-payment mini-form lives inline on the detail page**, not a separate route: it's a small, repeatable action (multiple partial payments accumulate toward `AmountDue` via the same `POST /record-payment` call - `AmountPaid` in the request is the amount being paid NOW, added to the running total server-side, not a new total), not a distinct resource needing its own detail view. It's hidden once the payment is Paid or Cancelled - a frontend-only guardrail (the backend doesn't actually block recording more money against an already-Paid payment) to prevent an accidental overpayment through the UI, not a business rule being enforced. Manually verified the full accumulation live: created a Pending £1050 payment, recorded a £600 partial (status flipped to Partially Paid, form stayed visible), then the remaining £450 (status flipped to Paid, form correctly disappeared, only Cancel remained), then cancelled it (status Cancelled, all actions gone, "This payment has been cancelled" note shown) - confirming `calculate_payment_status`'s live-computed status is reflected correctly through every transition without a page reload.

**"Payment history by tenancy" (Prompt 22) is `RentPaymentsListPage` itself, reused rather than a new page**: `TenancyDetailPage` gained a `Payment history` button in its `PageHeader` actions linking to `/rent-payments?tenancyId={id}`. The list page reads that query param once via `useSearchParams` on mount into a `tenancyFilter` state (not exposed as its own dropdown control - Property/Tenant/Status/due-date-range are the visible `FilterPanel` fields), fetches the tenancy's property+tenant name via `tenancyService.getTenancy` for a friendly label, and shows a dismissible `.filter-banner` ("Showing payment history for tenancy X — Clear") above the table. Passing `tenancy_id` straight through to the API is more precise than approximating "history" via property+tenant filters, and reuses the exact backend param `RentPaymentRepository.list` already supports.

**CSV export (Prompt 22) is entirely client-side** - `utilities/csvExport.js`'s `downloadCsv(filename, columns, rows)`, no library added (there's no `/export` route anywhere in `app/api/routes`, confirmed by grep before building this). Builds a CSV string from whatever rows are already loaded on the page and triggers a browser download via a `Blob` + temporary `<a download>`. Added to all three rent-payment list pages (`RentPaymentsListPage`, `RentPaymentOverduePage`, `RentPaymentDueThisMonthPage`); verified for real via a `HTMLAnchorElement.prototype.click` hook in the browser console rather than just trusting the code, confirming a real `blob:` URL and the correct filename fire on click. **This utility is meant to be reused for the Dashboard's own CSV export requirement later, not reinvented.**

Manually created, partially paid, fully paid, and cancelled a throwaway rent payment via the browser to verify the whole flow (including the `Payment history` link-through, which correctly showed it alongside PM-0006/Grace Kim's other 4 seeded payments), then deleted it via `sqlcmd` (`DELETE FROM RentPayments WHERE RentPaymentId = <id>`) to restore the seeded count of 30. Confirmed role-gating: MaintenanceEmployee bounced to `/unauthorized` from `/rent-payments` itself (matching `CAN_VIEW_RENT_PAYMENTS` excluding them, the same shape as Tenancies/Landlords); ReadOnly can view the list, Overdue/Due-this-month pages, and export CSV, but sees no `+ New Payment` button and no record-payment form/Cancel button on a payment's detail page.
